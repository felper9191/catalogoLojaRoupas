...
import { vestidos } from "./ListaTodosProdutos";
import HeaderProduto from "../Components/HeaderProduto";
import { useSearch } from "../Context/SearchContext";
import { useLocation } from 'react-router-dom';
import { useEffect } from 'react';

const ListaProdutosVestidos = () => {

    const [maisVestidos, setMaisVestidos] = useState(8);
    const { searchTerm, setSearchTerm } = useSearch();
    const location = useLocation();

    const carregarMais = () => {
        setMaisVestidos(maisVestidos + 8);
    }
    ...
    const filteredList = vestidos
        .filter((item) => item.nome.toLowerCase().includes(searchTerm.toLowerCase()))
        .slice(0, maisVestidos);

    return (
        <div className="listaDeProdutos">
            <HeaderProduto nomeProduto = "Vestidos" />
...
export default ListaProdutosVestidos;